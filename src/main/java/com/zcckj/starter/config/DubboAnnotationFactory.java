...
import com.zcckj.starter.annotation.DubboService;
...
/**
 * Created by 李朝衡 on 2017/5/19.
 */
public class DubboAnnotationFactory extends AbstractConfig implements DisposableBean, BeanFactoryPostProcessor, BeanPostProcessor, ApplicationContextAware {
    ...
    public Object postProcessAfterInitialization(Object bean, String beanName)
        ...
        Class<?> clazz = this.getBeanClass(bean);
        ...
        if (service != null) {
            ...
            if (StringUtils.isEmpty(service.group())) {
                if (env.getActiveProfiles() != null && env.getActiveProfiles().length > 0) {
                    serviceConfig.setGroup(env.getActiveProfiles()[0]);
                } else {
                    serviceConfig.setGroup(env.getDefaultProfiles()[0]);
                }
            } else {
                serviceConfig.setGroup(env.getProperty(service.group()));
            }
    ...
    private Object refer(Reference reference, Class<?> referenceClass) { //method.getParameterTypes()[0]
        ...
        if(StringUtils.isEmpty(reference.group())){
            if (env.getActiveProfiles() != null && env.getActiveProfiles().length > 0) {
                rg = env.getActiveProfiles()[0];
            } else {
                rg = env.getDefaultProfiles()[0];
            }
        }else {
            rg = env.getProperty(reference.group());
        }
        String key = rg + "/" + interfaceName + ":" + reference.version();
